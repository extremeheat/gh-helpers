...
declare module "gh-helpers" {
  ...
  interface HookOnRepoCommentPayload {
  ...
  interface HookOnUpdatedPRPayload {
    // Full data for the repo that triggered the workflow
    repository: Repository,
    // The full name fro the repo that the workflow ran on (owner/repo)
    repoId: string,
  ...
  interface HookOnWorkflowDispatchPayload {
    // Full data for the repo that triggered the workflow
    repository: Repository,
    // The full name fro the repo that the workflow ran on (owner/repo)
    repoId: string,
  ...
  interface ArtifactsAPI {
    ...
    deleteId(id: number): Promise<unknown>
    deleteIdFrom(owner: string, repo: string, id: number): Promise<unknown>
    downloadId(id: number, path: string): Promise<unknown>
    downloadIdFrom(owner: string, repo: string, id: string, path: string): Promise<unknown>
